...
const PersonalTab = ({ formData, onChange, shiftTypes, departmentTypes, branchTypes, designationTypes, id }) => {
  ...
  useEffect(() => {
    //if (formData.employeeShift?.[0]?.shift_id) {
     if (formData.employeeShift?.shift_id) {
      const shift = shiftTypes.find(
        //(shift) => shift.id === formData.employeeShift[0].shift_id
       (shift) => String(shift.id) === String(formData.employeeShift.shift_id)
      );
      if (shift) {
...
const handleShiftChange = (e) => {
  ...
  const updatedEmployeeShift = {
    shift_id: shiftId,
    week_off: formData.employeeShift?.week_off || ''
  };
  onChange({ target: { name: "employeeShift", value: updatedEmployeeShift } });

  // Set selectedShift and update shiftTime/workHours as before
  const shift = shiftTypes.find((shift) => String(shift.id) === String(shiftId));
  if (shift) {
    setSelectedShift(shift);
    const shiftStart = new Date(`1970-01-01T${shift.start}:00`);
    let shiftEnd = new Date(`1970-01-01T${shift.end}:00`);
  ...
  return (
    <div className="personal-tab">
      ...
      {/* <div className="row">
        <div className="col-md-6 col-lg-6 col-xl-4 mt-0 pt-4">
        ...
        <div className="col-md-6 col-lg-6 col-xl-4 mt-4 mb-4">
      ...
      <div className="col-md-4 col-lg-3 col-xl-3">
        ...
        <div className="col-md-4 col-lg-3 col-xl-3">
          ...
          {/* <CustomDropdown
  label="Shift Type"
  name="employeeShift.shift_id"
  options={shiftTypes} // Updated dropdown options
  value={formData.employeeShift?.shift_id || ''} // Access shift_id directly
  onChange={(e) => {
    const shiftId = e.target.value;
    const updatedEmployeeShift = {
      ...formData.employeeShift,
...
/> */}
        ...
        <div className="col-md-4 col-lg-3 col-xl-3">
          <TextInput
            label="Week Off"
            name="employeeShift.week_off"
            //value={formData.employeeShift.week_off}
            value={formData.employeeShift?.week_off}
            onChange={onChange}
            required
